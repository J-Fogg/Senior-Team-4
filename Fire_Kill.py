...
greenLower = (160,150,100)
greenUpper = (180,255,230)
...
while True:
    ...
    if len(cnts) > 0 :
        ...
        MinY = sorted(contt, key=lambda x: x[1])[:50]
        ValueY = int(MaxY[0][1] - MinY [0][1])
        ...
        Pixel = (434.9249*2.375)/ValueY
        #Pixels = (2.375 * Pixel)/ ValueY
        #Area = (ValueY*(RightX[0][0]-LeftX[0][0]))/2
        #Bottom = (MaxY[0][1] - MinY[0][1])*2/3 + MinY[0][1]
        #Top = (MaxY[0][1] - MinY[0][1])/3 + MinY[0][1]
        #max(contt, key=Lambda x:, Bottom)
        #NewObject = (-.16567*ValueY + 33.43688)
        print(Pixel)
        if (Object < 50):
